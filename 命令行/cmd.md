...
`iconv -l` 列出已知的编码
`iconv -f fromEncoding -t toEncoding inputFile -o outputFile` 将一个文件的编码从 fromEncoding 变为 toEncoding


##### 基本命令
`arch` 显示机器的处理器架构：arm64
`uname -m` 显示机器的处理器架构：arm64
`uname -r` 显示正在使用的内核版本：20.5.0
`date` 当前系统日期
`cal` 当月日历表
